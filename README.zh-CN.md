...
非秘密配置可以放在 `[mcp_servers.scythe_context.env]`：
...
`WSLENV` 是 WSL interop 规则，不是 Codex 专用字段。只有在 Windows Codex App + WSL repo 模式需要让 WSL 把变量转交给 Windows Node 时才需要。若设置了额外 Gemini 变量，记得把它们加进 `WSLENV`：
