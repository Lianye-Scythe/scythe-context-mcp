...
為了控制 Codex token 消耗，`repo_index_status`、`repo_related_files` 與 `repo_reindex` 預設回傳 compact 摘要，包含決策必要資訊與估算輸出 token；需要完整診斷資料、完整 skipped file list 或 provider capability raw details 時，才使用 `response_mode="full"`。
